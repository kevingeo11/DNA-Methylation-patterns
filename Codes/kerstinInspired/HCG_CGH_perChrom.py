...
def create_intersect_chr(inp, out, chr):
    with open(inp) as f:
        reader = csv.reader(f, delimiter = '\t')
...
outpath_processing = main_path + 'Data\\Per Chrom\\'

inter_out_promo_NOMe = outpath_processing + 'NOMe_intersect.csv'
inter_out_promo_WGBS = outpath_processing + 'WGBS_intersect.csv'

chr = 'chr3'
chr_out_promo_NOMe = outpath_processing + 'NOMe_intersect_{}.csv'.format(chr)
chr_out_promo_WGBS = outpath_processing + 'WGBS_intersect_{}.csv'.format(chr)

if True:
    print('NOMe')
    create_intersect_chr(inter_out_promo_NOMe,chr_out_promo_NOMe, chr)
    
    print('WGBS')
    create_intersect_chr(inter_out_promo_WGBS,chr_out_promo_WGBS, chr)

#pandas intersect outfiles
outfile_pandas_NOMe = outpath_processing + 'pandas_promoter_NOMe_{}.csv'.format(chr)
outfile_pandas_WGBS = outpath_processing + 'pandas_promoter_WGBS_{}.csv'.format(chr)

#random files for sliding window
inter_out_promo_WGBS_RANDOM = outpath_processing + 'intersectBed_UCSC_complete_promoter_WGBS_RANDOM_{}.csv'.format(chr)
outfile_pandas_WGBS_RANDOM = outpath_processing + 'pandas_promoter_WGBS_RANDOM_{}.csv'.format(chr)
...
if True:
    print('NOMe')
    make_pandas_df(chr_out_promo_NOMe,outfile_pandas_NOMe)
    
    print('WGBS')
    make_pandas_df(chr_out_promo_WGBS,outfile_pandas_WGBS)
...
if True:
    ...
    randomize_genome_methylation(df_promo_nuc_WGBS_pandas, chr_out_promo_WGBS, inter_out_promo_WGBS_RANDOM)    
...
if True:
    ...
    plot_out = outpath_processing + 'NOMe_promoter_average_100-GpC_meth_{}.pdf'.format(chr)
...
if True:
    ...
    plot_out = outpath_processing + "WGBS_promoter_distances_CpGs_{}.pdf".format(chr)
    plot_WGBS_CpG_distances_in_promoter(df_inter_promo_WGBS,plot_out)
    
    #calc 100-CgG methylation
    plot_out = outpath_processing + "WGBS_promoter_average_CpG_meth_{}.pdf".format(chr)
...
if True: 
    ...
    plot_out = outpath_processing + "WGBS_RANDOM_distances_CpGs_{}.pdf".format(chr)
    plot_WGBS_CpG_distances_in_promoter(df_inter_promo_WGBS_RANDOM,plot_out)
    
    plot_out = outpath_processing + "WGBS_RANDOM_promoter_average_CpG_meth_{}.pdf".format(chr)
...
if True:
    df_inter_promo_WGBS = pd.read_csv(outfile_pandas_WGBS, index_col=0)
    df_inter_promo_WGBS_RANDOM = pd.read_csv(outfile_pandas_WGBS_RANDOM, index_col=0)
    plot_out = outpath_processing + "WGBS_AND_RANDOM_promoter_average_CpG_meth_{}.pdf".format(chr)
